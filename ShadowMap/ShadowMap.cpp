...
INT WINAPI wWinMain( HINSTANCE, HINSTANCE, LPWSTR, int )
{
	...
	vFromPt	= D3DXVECTOR3( 0.0f, 30.0f, 0.0f );
	vLookatPt = D3DXVECTOR3( 0.0f, -1.0f, 0.1f );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	{	//PSM
		...
		D3DXMatrixMultiply(&matEyeView_EyeProjection_LightView, g_VCamera.GetViewMatrix(), g_VCamera.GetProjMatrix());	//카메라 뷰 * 카메라 투영
		D3DXMatrixMultiply(&matEyeView_EyeProjection_LightView, &matEyeView_EyeProjection_LightView, &mLightView );	//카메라 뷰 * 카메라 투영* 광원 뷰		
	...
	{	//mViewToLightProj에는 이미 카메라 view가 포함되어있다
		...
		D3DXMatrixMultiply(&mViewToLightProj, &mViewToLightProj, g_VCamera.GetProjMatrix());	//카메라 뷰 * 카메라 투영
		D3DXMatrixMultiply(&mViewToLightProj, &mViewToLightProj, &mLightView );		//카메라 뷰 * 카메라 투영* 광원 뷰		
